    ...
    def get(self, request, *args, **kwargs):
        ...
        if not df.empty:
            ...
            for i, row in unique_issuetype.iterrows():
                data = df[df['issuetype_name'] == row['issuetype_name']][[
                    'complaint_name', 'description', 'extension', 'status', 'floor', 'building']]
                issues = [dict(row) for index, row in data.iterrows()]
...
class SurveyDashboardAPIView(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        inpatient = qs.filter(flag=True).order_by("-id")
        inpatient_count = inpatient.count()
        outpatient = qs.filter(flag=False).order_by("-id")
